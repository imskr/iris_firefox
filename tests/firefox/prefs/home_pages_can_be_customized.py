...
class Test(BaseTest):
    ...
    def run(self, firefox):
        ...
        assert preferences_are_opened, 'Preferences#Home page is opened'
        ...
        preference_region = Region(preference_location.x, preference_location.y - (preference_region_offset // 2),
                                   Screen.SCREEN_WIDTH // 2, preference_region_offset * 2)

        home_is_default = exists(default_new_tab_setting_home_pattern, region=preference_region)
        assert home_is_default, 'Preferences page is opened, Firefox Home is set as default '

        click(default_new_tab_setting_home_pattern, region=preference_region)

        custom_url_option_exists = exists(custom_url_option_pattern)
        assert custom_url_option_exists, '"Custom URL" option is displayed'

        click(custom_url_option_pattern)

        url_field_displayed = exists(url_field_pattern)
        assert url_field_displayed, 'URL field is displayed'
        ...
        assert custom_webpage_displayed_as_default_in_new_window, 'Custom page was set as default'
